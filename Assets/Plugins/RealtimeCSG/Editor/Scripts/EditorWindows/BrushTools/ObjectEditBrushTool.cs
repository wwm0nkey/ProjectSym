...
namespace RealtimeCSG
{
	internal sealed class ObjectEditBrushTool : ScriptableObject, IBrushTool
	{
		...
		Vector3[]					projectedBounds				= new Vector3[8];
		List<GameObject>			dragGameObjects				= new List<GameObject>();
		int							hoverOverBrushIndex;
		...
		#region Misc state
		bool	toolIsEnabled		= false;
		bool	shouldHideTool		= false;
		#endregion
		...
		void UpdateTargetBounds()
		{
			...
			BoundsUtilities.GetBoundsVertices(targetLocalBounds, targetLocalPoints);
		...
		public void SetPivotCenter(Vector3 newCenter, bool registerUndo = true)
		{
			...
			if (difference != Vector3.zero)
			{
				...
				{
					topTransform.position = topTransform.position + difference;
				}
		...
		void UpdateGrid(Camera camera)
		{
			...
			if (hoverOnBoundsCenter != -1)
			{
				...
				Grid.SetupRayWorkPlane(worldOrigin, worldDirection, ref movePlane);
			} else
			{
				Grid.SetupWorkPlane(originalPoint, ref movePlane);
			}
		}
		...
		void UpdateGridOrientation(Vector2 mousePosition)
		{
			...
			//prevYMode		= Grid.YMoveModeActive;
			UpdateTargetInfo();
		}
		...
		bool MoveObjects(Transform[] transforms, Vector3 offset, out Vector3 snappedOffset, bool doSnapping)
		{
			...
			if (doSnapping)
			{
				...
				offset = Grid.SnapDeltaToGrid(offset, localPoints, snapToSelf: true);
			} else
			{
				offset = Grid.HandleLockedAxi(offset);
			}
		...
		void MoveBoundsCenter(int boundsCenterIndex, Vector3 offset, out Vector3 snappedOffset, bool doSnapping)
		{
			...
			if (doSnapping)
			{
				...
				worldOffset			= Grid.SnapDeltaToRay(new Ray(worldLineOrg, worldLineDir), worldOffset, worldPoints, snapToSelf: true);
		...
		void MoveBoundsEdge(int edgeCenter, Vector3 offset, out Vector3 snappedOffset, bool doSnapping)
		{
			...
			if (doSnapping)
			{
				...
				worldOffset = Grid.SnapDeltaToGrid(worldOffset, worldPoints, snapToSelf: true);
				localOffset = activeSpaceMatrices.activeWorldToLocal.MultiplyVector(worldOffset);
			} else
			{
				worldOffset = Grid.HandleLockedAxi(worldOffset);
				localOffset = activeSpaceMatrices.activeWorldToLocal.MultiplyVector(worldOffset);
			}
			if (float.IsNaN(localOffset.x) ||
				float.IsNaN(localOffset.y) ||
						...
						if (!Camera.current.orthographic && CSGSettings.GridVisible)
						{
							...
							if (!mouseIsDragging || (Tools.current != Tool.Rotate))
							{
								for (int i = 0; i < indices.Length; i++)
								{
									...
									var pt1 = Grid.CurrentGridPlane.Project(pt0);
								...
								if (activeTransform != null)
								{
									...
									//if (!Camera.current.orthographic)
									{
												...
												*/
												...
												for (int t = 0; t < targetBoundEdgeIDs.Length; t++)
												{
														...
														Tools.current == Tool.Rect)
													{
														...
														{
															distance = Mathf.Min(
																			GUIStyleUtility.DistanceToLine(cameraPlane, point1, point2) * 3.0f,
																			HandleUtility.DistanceToCircle(matrix.MultiplyPoint(renderLocalEdgePoints[t]), renderLocalEdgePointSizes[t]) * 0.5f);
															HandleUtility.AddControl(targetBoundEdgeIDs[t], distance);
														}
													} else
													{
														distance = GUIStyleUtility.DistanceToLine(cameraPlane, matrix.MultiplyPoint(point1), matrix.MultiplyPoint(point2));
														HandleUtility.AddControl(targetBoundEdgeIDs[t], distance);
													}
												}
											}
										...
										if (nearestControl > 0 && !foundControl)
										{
											...
											//	view_is_2D)
											if (//!Grid.YMoveModeActive &&
												(Tools.current == Tool.Scale || Tools.current == Tool.View || Tools.current == Tool.Rotate))
											{
												for (int t = 0; t < targetBoundEdgeIDs.Length; t++)
												{
					...
					switch (type)
					{
						case EventType.MouseDrag:
						{
							...
							if (firstMove)
							{
								...
								BoundsUtilities.GetBoundsVertices(targetLocalBounds, projectedBounds);
							}
							...
							if (!float.IsNaN(worldIntersection.x) && !float.IsNaN(worldIntersection.y) && !float.IsNaN(worldIntersection.z))
							{
								if (((prevModifiers & EventModifiers.Shift) != (SelectionUtility.CurrentModifiers & EventModifiers.Shift) 
									...
									Grid.ForceGrid && startCamera != null)
								{
							...
							if (useRaySnapping)
							{
								...
								try
								{
									...
									{ 
										...
										Grid.SetForcedGrid(intersection.plane);
										...
										if (doSnapping)
										{
											...
											hoverPosition += Grid.SnapDeltaToGrid(MathConstants.zeroVector3, localPoints);
										}
		...
		public void OnInspectorGUI(EditorWindow window)
		{
			...
			{
				ObjectToolGUI.OnInspectorGUI(window);
			}
