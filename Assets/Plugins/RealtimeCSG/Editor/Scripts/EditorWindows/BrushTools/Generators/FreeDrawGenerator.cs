...
{	
	...
	{	
		...
		internal override bool UpdateBaseShape(bool registerUndo = true)
		{
			...
			var	curvedMaterials		= new Material[vertexCount];
			...
			{
				var material	= settings.curveEdgeHandles[i].Material;
        ...
        internal override bool StartExtrudeMode(bool showErrorMessage = true)
		{
			...
			var usedSmoothingGroupIndices = SurfaceUtility.GetUsedSmoothingGroupIndices();
			var curvedMaterials		= new Material[outlineVertices.Length];
				...
				switch (Event.current.GetTypeForControl(id))
				{
					...
					case EventType.MouseDrag:
					{
							...
							(Tools.viewTool == ViewTool.None || Tools.viewTool == ViewTool.Pan))
						{
							...
							var alignedPlane	= new CSGPlane(Grid.CurrentWorkGridPlane.normal, settings.backupCurve.Points[i]);
							...
							visualSnappedGrid = Grid.FindAllGridEdgesThatTouchPoint(difference + settings.backupCurve.Points[i]);
