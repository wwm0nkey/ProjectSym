...
namespace RealtimeCSG
{
	internal abstract class GeneratorBase : ScriptableObject//, IBrushGenerator
	{
		...
		public virtual void Init()
		{
			...
			originalGridRotation	= Grid.ForcedGridRotation;
			originalGridCenter		= Grid.ForcedGridCenter;
			originalForceGrid		= Grid.ForceGrid;
		...
		{
			Grid.ForcedGridRotation = originalGridRotation;
			Grid.ForcedGridCenter	= originalGridCenter;
			Grid.ForceGrid			= originalForceGrid;
		}
		...
		protected void SetBrushTransformation(Transform transform)
		{
			...
			transform.rotation = Quaternion.FromToRotation(MathConstants.upVector3, direction) *
									Quaternion.Inverse(parentModel.transform.rotation);// * fromGridQuaternion;
		...
		{
			var gridSpaceNormal = Grid.VectorToGridSpace(buildPlane.normal);
			...
			gridTangent  = Grid.VectorFromGridSpace(gridSpaceTangent);
			gridBinormal = Grid.VectorFromGridSpace(gridSpaceBinormal);
			
			toGridMatrix		= Grid.ToGridSpaceMatrix();
			fromGridMatrix		= Grid.FromGridSpaceMatrix();
			toGridQuaternion	= Grid.ToGridSpaceQuaternion();
			fromGridQuaternion	= Grid.FromGridSpaceQuaternion();
		}
