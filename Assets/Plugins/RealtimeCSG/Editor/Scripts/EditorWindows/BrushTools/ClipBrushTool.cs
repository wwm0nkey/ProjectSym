			...
			{ 
				...
				for (int t = backupData.Length - 1; t >= 0; t--)
				{
					...
					//if (brush.ControlMesh.Generation != backupData[t].controlMesh.Generation)
					{
						...
						InternalCSGModelManager.RegisterMaterials(model, brush.Shape, false);
			...
			{ 
				...
				for (int t = offset; t < backupData.Length; t++)
				{
					...
					if (needCopies && backupData[t].keepCopy && backupData[t].brushCopy && backupData[t].brushCopy.gameObject.activeSelf)
					{
						...
						InternalCSGModelManager.RegisterMaterials(backupData[t].model, backupData[t].brushCopy.Shape, false);
					...
					InternalCSGModelManager.RegisterMaterials(backupData[t].model, backupData[t].brush.Shape, false);					
					if (backupData[t].removeOnCommit)
						continue;
...
//				prevClipPlaneValid = clipPlaneValid;
		...
		void UpdateMouseIntersection()
		{
			...
			} else
			{
				...
				if (pointsUsed > 0 && Camera.current != null && Camera.current.orthographic)
				{
					movePlane = new CSGPlane(Camera.current.transform.forward, points[0]);
				} else
...
//		MouseCursor lastUsedCursor = MouseCursor.Arrow;
		...
		void CreateFirstPoint()
		{
			...
			if (Camera.current != null && Camera.current.orthographic)
				normal = -Camera.current.transform.forward;
			...
			if (currentOnBrush != null)
			{
				...
				{
					var ray_start	= intersectionPoint - (normal * Camera.current.farClipPlane);
					var ray_end		= intersectionPoint + (normal * Camera.current.farClipPlane);
			...
			if ((points[2] - points[1]).sqrMagnitude < 0.1f)
			{
				...
				var snapVector = Grid.CurrentGridSnapVector;
...
//			onHandle = false;
				...
				switch (Event.current.type)
				{
					...
					case EventType.MouseUp:
					{
						if (GUIUtility.hotControl == 0 && Event.current.button == 0)
						{
								...
								Event.current.modifiers == EventModifiers.None)
							{
								...
								if (Camera.current != null && Camera.current.orthographic)
								{
									intersectionPoint = new CSGPlane(Camera.current.transform.forward, points[0]).Project(intersectionPoint);
								}
								points[2] = intersectionPoint;
							} else
							if (!mouseDragged)
					...
					case EventType.Repaint:
					{
						...
						for (int i = 0; i < pointsShown; i++)
						{
							...
							PaintUtility.SquareDotCap(ids[i], points[i], Camera.current.transform.rotation, sizes[i]);
						}
						Handles.color = temp;
					...
					switch (type)
					{
						...
						case EventType.MouseDrag:
						{
							if (GUIUtility.hotControl == ids[i])
							{
								...
								if (Camera.current != null && Camera.current.orthographic)
									newPoint = new CSGPlane(Camera.current.transform.forward, points[i]).Project(newPoint);
		...
		public void OnInspectorGUI(EditorWindow window)
		{
			...
			{
				ClipToolGUI.OnInspectorGUI(window);
			}
