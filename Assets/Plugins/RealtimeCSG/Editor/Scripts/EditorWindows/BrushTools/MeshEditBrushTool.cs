	...
	internal sealed class MeshEditBrushTool : ScriptableObject, IBrushTool
	{
		...
		void UpdateChamferIntersectionPoint()
		{
			...
			for (int p0 = 0; p0 < chamferPlanes.Length; p0++)
			{
				...
				Grid.SetForcedGrid(chamferPlanes[p0]);
			...
			switch (chamferMode)
			{
				...
				case ChamferMode.Point:
				{
					...
					if (doSnapping)
					{
						chamferIntersectionPoint = intersection + Grid.SnapDeltaToGrid(intersection - projectedPoint, new Vector3[] { intersection });//, snapToSelf: true);
					} else
						chamferIntersectionPoint = intersection + Grid.HandleLockedAxi(intersection - projectedPoint);
					break;
				}
