...
//			var used_planes  = new HashSet<int>();
		...
		public static void FixInvalidPolygons(ControlMesh controlMesh, Shape shape)
		{
			...
			for (var c = 0; c < checkPolygons.Count; c++)
			{
				...
				{
					Debug.Log("polygon.edgeIndices.Length == 1!");
					break;
				} else
			...
			RemoveUnusedMaterials(controlMesh, shape);
			...
			if (shape.Surfaces.Length		!= 0) shape.Surfaces		= new Surface[0];
			if (shape.TexGens.Length		!= 0) shape.TexGens			= new TexGen[0];
			if (shape.TexGenFlags.Length	!= 0) shape.TexGenFlags		= new TexGenFlags[0];
			if (shape.Materials.Length		!= 0) shape.Materials		= new Material[0];
		}
		...
		public static int InsertEdgeBetweenVertices(ControlMesh controlMesh, Shape shape, short vertexIndex1, short vertexIndex2)
		{
			...
			for (var polygonIndex = 0; polygonIndex < controlMesh.Polygons.Length; polygonIndex++)
			{
				...
				{
					Debug.Log("next_index1 == edge_index2 || next_index2 == edge_index1");
					return -1;
				}
		...
		public static int InsertEdgeBetweenEdges(ControlMesh controlMesh, Shape shape, int edgeIndex1, int edgeIndex2)
		{
			...
			if (edgeIndex1 == -1 ||
				edgeIndex2 == -1)
			{
				Debug.Log("edgeIndex1 == -1 || edgeIndex2 == -1");
				return -1;
			}
			...
			{
				Debug.Log("polygon_index == -1");
				return -1;
			}

			if (controlMesh.GetEdgePolygonIndex(edgeIndex2) != edgePolygonIndex)
			{
				Debug.Log("controlMesh.GetEdgePolygonIndex(edgeIndex2) != polygon_index");
				return -1;
			}
		...
		private static int CopyTexGen(Shape shape, int srcTexGenIndex)
		{
			...
			texGen.MaterialIndex = -1;
			ArrayUtility.Add(ref shape.TexGens, texGen);
			ArrayUtility.Add(ref shape.TexGenFlags, shape.TexGenFlags[srcTexGenIndex]);
			ArrayUtility.Add(ref shape.Materials, shape.Materials[srcTexGenIndex]);
			return shape.TexGens.Length - 1;
		}

		public static void RemoveUnusedMaterials(ControlMesh controlMesh, Shape shape)
		{
			...
			var usedMaterials	= new bool[shape.Materials.Length];
			for (var p = 0; p < polygons.Length; p++)
			{							
				usedMaterials[polygons[p].TexGenIndex] = true;
			}

			var indexConversion = new int[shape.Materials.Length];

			var materialCount = usedMaterials.Length;
			for(var i = 0; i < materialCount; i++)
			{
				...
				shape.Materials[i] = shape.Materials[found];
				shape.TexGenFlags[i] = shape.TexGenFlags[found];
				shape.TexGens[i] = shape.TexGens[found];

				shape.Materials[found] = null;
				usedMaterials[i] = true;
				usedMaterials[found] = false;
				materialCount--;
			}

			if (materialCount == usedMaterials.Length)
				return;
			...
			for (var i = usedMaterials.Length - 1; i >= 0; i--)
			{
				if (usedMaterials[i])
					break;

				ArrayUtility.RemoveAt(ref shape.Materials, i);
			...
			for (int i = 0, maxI = controlMesh.Polygons.Length - 1; i < maxI; i++)
			{
				for (int j = i + 1, maxJ = controlMesh.Polygons.Length; j < maxJ; j++)
				{
					...
					{
						var tempMaterial = shape.Materials[i];
						shape.Materials[i] = shape.Materials[j];
						shape.Materials[j] = tempMaterial;
		...
		public static void FixTexGens(ControlMesh controlMesh, Shape shape)
		{
			...
			for (var t = shape.TexGens.Length - 1; t >= 0; t--)
			{
				...
				ArrayUtility.RemoveAt(ref shape.Materials, t);
		...
		public static bool CutMesh(ControlMesh controlMesh, Shape shape, CSGPlane cuttingPlane, ref List<int> intersectedEdges)
		{
			...
			for (var p = controlMesh.Polygons.Length - 1; p >= 0; p--)
			{
				...
				{
					Debug.Log("Number of edges criss-crossing the plane boundary is 1 (should not be possible)!?");
					return false;
				}
...
//					var edge_index_count = polygon.edgeIndices.Length;
				...
				{
					Debug.Log("Found an uneven number of edge-plane traversals??");
					return false;
				}
		...
		public static bool FindAndDetachSeparatePiece(ControlMesh controlMesh, Shape shape, CSGPlane cuttingPlane, out ControlMesh newControlMesh, out Shape newShape)
		{
			...
			var newPlaneArrayB		= new Surface[polygonLengthB];
			{
				...
				for (var i = 0; i < lookupPolygonIndicesA.Length; i++)
				{
					...
					if (lookupPolygonIndicesA[i] == -1)
					{
						...
						{
							Debug.Log("texgen_index < 0 || texgen_index >= new_texgen_lookupB.Length");
						}
						if (newTexgenLookupB[texgenIndex] == 0)
						{
							newTexgenListB  .Add(shape.TexGens[texgenIndex]);
							newMaterialListB.Add(shape.Materials[texgenIndex]);
					...
					{ 
						...
						if (newTexgenLookupA[texgenIndex] == 0)
						{
							...
							newMaterialListA.Add(shape.Materials[texgenIndex]);
			...
			newShape = new Shape
			{
				...
				Materials	= newMaterialListA.ToArray()
			};
			...
			shape.Materials			= newMaterialListB.ToArray();
		...
		private static void ClipOffTriangle(ControlMesh controlMesh, Shape shape, short[] vertexIndices, short polygonIndex)
		{
			...
			{
				Debug.Log("invalid order of vertices");
				return;
			}
...
//			var currVertexIndex		= controlMesh.GetVertexIndex(currEdgeIndex);
		...
		public static bool Triangulate(CSGBrush brush, ControlMesh controlMesh, Shape shape)
		{
			...
			for (var p = controlMesh.Polygons.Length - 1; p >= 0; p--)
			{
				...
				while (foundTriangles != null && foundTriangles.Length > 0)
				{
					...
					if (found == -1)
					{
						Debug.Log("Failed to find appropriate triangle to clip during triangulation");
						break;
					}
			...
			*/
			...
			for (var p = (short)(controlMesh.Polygons.Length - 1); p >= 0; p--)
			{
				...
				for (var e = 0; e < currEdges.Length; e++)
				{
					...
					{
						Debug.Log("edge with index " + currIndex + " is invalid", brush);
						continue;
					}
					var twinIndex	= controlMesh.GetTwinEdgeIndex(currIndex);
					...
					{
						Debug.Log("edge " +currIndex + " with twin-index " + twinIndex + " is invalid", brush);
						continue;
					}
					...
					if (p == twinPolygonIndex)
					{
						...
						if (srcIndex1 == -1 ||
							srcIndex2 == -1)
						{
							Debug.Log("src_index1 == -1 || src_index2 == -1");
							continue;
						}
					...
					{
						Debug.Log("src_index < 0 || src_index >= curr_edges.Length");
						continue;
					}
		...
		{
			public EdgeIntersection(int edgeIndex, short vertexIndex, int planeIndexA, int planeIndexB)
			{
				VertexIndex	= vertexIndex;
				EdgeIndex		= edgeIndex;
				TwinIndex		= -1;
		...
		public static ControlMesh CreateFromShape(Shape shape)//, Vector3 const& translation)
		{
		//	var surfaces		= shape.surfaces;
			...
			{
				shape.Materials = new Material[shape.Surfaces.Length];
			} else
			while (shape.Materials.Length < shape.Surfaces.Length)
			{
				ArrayUtility.Add(ref shape.Materials, shape.Materials[shape.Materials.Length - 1]);
			}
				...
				for (var p2 = p1 + 1; p2 < surfaceCount - 1; p2++)
				{
					...
					for (var p3 = p2 + 1; p3 < surfaceCount; p3++)
					{
						...
						for (var p4 = 0; p4 < surfaceCount; p4++)
						{
							...
							if (side < -MathConstants.DistanceEpsilon) // inside
								continue;

							if (side > MathConstants.DistanceEpsilon)	// outside
			...
			for (var i = 0; i < pointIntersections.Count; i++)
			{
				for (var j = i + 1; j < pointIntersections.Count; j++)
				{
					...
					edgeIntersections.Add(new EdgeIntersection(edgeIndexA,
																 pointIntersections[i].VertexIndex,
																 foundPlanes[0],
																 foundPlanes[1]));
					var intersectionB = edgeIntersections.Count;
					edgeIntersections.Add(new EdgeIntersection(edgeIndexB,
																 pointIntersections[j].VertexIndex,
																 foundPlanes[0],
																 foundPlanes[1])); 
			...
			for (var i = pointIntersections.Count - 1; i >= 0; i--)
			{
				...
				for (var j = 0; j < pointEdgeIndices.Count - 1; j++)
				{
					...
					for (var k = j + 1; k < pointEdgeIndices.Count; k++)
					{
						...
						if (Vector3.Dot(direction, edge2Plane.normal) < 0)
						{
...
//				var polygon_bounds	= polygonBounds[s];
			...
			RemoveUnusedMaterials(newMesh, shape);
			

			return !Validate(newMesh, shape) ? null : newMesh;
		}
		...
		public static ControlMesh EnsureValidControlMesh(CSGBrush brush)
		{
			...
			{
				if (brush.Shape != null)
					controlMesh = CreateFromShape(brush.Shape);
		...
		{
			RemoveUnusedMaterials(controlMesh, srcShape);
			...
			ShapeUtility.CreateCutter(srcShape, controlMesh);
		...
		internal static bool UpdateBrushMesh(CSGBrush brush, ControlMesh controlMesh, Shape srcShape)
		{
...

			{
					...
					brushCache.childData.Model != null)
				{
					...
					InternalCSGModelManager.UnregisterMaterials(brushCache.childData.Model, brush.Shape, false);
				}
				...
				{
					updateModels.Add(brushCache.childData.Model);
					InternalCSGModelManager.RegisterMaterials(brushCache.childData.Model, brush.Shape, false);
				}
