...
namespace RealtimeCSG
{
	internal static class PaintUtility
	{
		...
		public static void DrawLine(Vector3 from, Vector3 to, float thickness, Color color)
		{
			if (Event.current.type != EventType.Repaint)
			{
				return;
			...
			var material = CustomThickWireMaterial;
			if (material == null)
				return;
			...
			if (material.SetPass(0))
			{
				...
				var	t = Vector3.Dot((cameraPosition - from), diff) / Vector3.Dot(diff, diff);
				if (t > 0.0f && t < 1.0f)
				{
					var pt2 = from + (t * diff);
					GL.MultiTexCoord(0, thickness0); GL.Vertex(from);
					GL.MultiTexCoord(0, thickness1); GL.Vertex(from);
					...
					GL.MultiTexCoord(0, thickness2); GL.Vertex(to);
					GL.MultiTexCoord(0, thickness3); GL.Vertex(to);
				} else
				{
					GL.MultiTexCoord(0, thickness0); GL.Vertex(from);
					GL.MultiTexCoord(0, thickness1); GL.Vertex(from);
		...
		public static void DrawLine(Matrix4x4 matrix, Vector3 from, Vector3 to, float thickness, Color color)
		{
			if (Event.current.type != EventType.Repaint)
			{
				return;
...
//			float height = cam.pixelRect.height * 0.5f;
		...
		public static void DrawLines(Matrix4x4 matrix, Vector3[] vertices, Int32[] indices, float thickness, Color color)
		{
			if (Event.current.type != EventType.Repaint)
			{
				return;
		...
		public static void DrawWorldText(Vector3 center, GUIContent content, GUIStyle style = null)
		{
			...
			Vector2 pos2D = HandleUtility.WorldToGUIPoint(center);
